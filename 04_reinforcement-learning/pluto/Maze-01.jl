...
# ╔═╡ efb5080e-0daf-11ed-0649-2927bc8beba0
using PlutoUI, GLMakie, NaNStatistics, Distributions

# ╔═╡ c16396f9-a83e-4875-aa7e-ae097eadf53c
md"""
```bash
# sysimage を読み込んで Julia 起動
$ julia -J $HOME/.julia/sysimage/GLMakie.dylib
```
"""

# ╔═╡ 34c71e6e-bb2b-4eeb-a164-1083c5fa7884
md"""
# 迷路問題

## 迷路の描画

Makie.jl を使って以下のような迷路を描画する
...
# ╔═╡ d8d6e384-71c0-4b2b-8dd2-b465b422f1cd
"""
...
# ╔═╡ f24cf616-3e44-481a-a4cf-cc577118bd67
"""
...
# ╔═╡ c4e5784f-a1b6-4a29-92a2-5ef0763126e9
"""
...
# ╔═╡ 8d7574db-9e87-4a7d-9e09-9d44b2ea67ab
fig = (() -> begin
	env = MazeEnv()
	fig, plt, ax, state_positions = plot(env)
	agent_pos = plot!(env, plt, state_positions[1])
...
# ╔═╡ 152426c4-6c4c-42fe-b088-f6a59ab67d7c
md"""
## エージェントの実装

表形式方策を用いて迷路問題のエージェントを実装する
"""

# ╔═╡ 5c0cb291-87ed-4d2a-a599-26e06463022f
"""
...
# ╔═╡ 3de56b2c-2319-4189-a6a0-a48deb08658d
"""
...
# ╔═╡ eb24a603-e5f6-4dd9-86bb-c4ce2c490e59
"""
...
# ╔═╡ 7727fe9b-064a-44e7-917a-4d46a59d5da9
tabler_policy_theta_ratio(MazeInitialTheta)

# ╔═╡ 40a853da-9da2-4c33-be5a-d312aaf03f0c
"""
	act!(env::MazeEnv, action)

エージェントに行動をとらせて迷路問題を次の状態に遷移
"""
act!(env::MazeEnv, action) = env.state += env.state_transitions[action]

# ╔═╡ f140795a-0a3e-40d6-9fc4-f92cbd3d0a2b
"""
	action_space(env::MazeEnv) -> actions::Tuple{Int}

迷路問題の行動空間取得
"""
action_space(env::MazeEnv) = env.actions

# ╔═╡ 83716f1b-2a0d-4029-b780-d965072b20d9
"""
	state_space(env::MazeEnv) -> states::Tuple{Int}

迷路問題の状態空間取得
"""
state_space(env::MazeEnv) = env.states

# ╔═╡ 0c922814-f4f1-4152-aec6-2c95fa5aacb6
"""
	current_state(env::MazeEnv) -> state

迷路問題の現在の状態取得
"""
current_state(env::MazeEnv) = env.state

# ╔═╡ 15291b9c-5bc4-4933-addc-df13e54453a7
"""
	init!(env::MazeEnv)
...
# ╔═╡ 3cd23557-8e79-4845-a0d0-ae667ae25c38
"""
	is_terminated(env::MazeEnv) -> ::Bool

迷路問題の終了判定
"""
is_terminated(env::MazeEnv) = env.state === Int(S8)

# ╔═╡ 77e6bb12-0df2-4097-ab30-91a3934e8216
md"""
## 強化学習のソルバー実装

ソルバーを実装する

ここでソルバーとは、「対象の環境が終了状態になるまで、方策に従ってエージェントに行動を取らせ続ける」ものと定義しておく

擬似コードで書くと以下のようなイメージである

```julia
solve!(環境, 方策, フック) = begin
    初期化!(環境)
    OnInit(フック, 環境, 方策) # エージェントの移動記録用配列の作成等を想定

    while !終了判定(環境)
        行動空間 = 行動空間取得(環境) # 行動空間: 行動の選択肢
        現在の状態 = 現在の状態取得(環境)
        次の行動 = 行動決定(方策, 行動空間, 現在の状態)

        継続フラグ = BeforeAction(フック, 環境, 方策, 現在の状態, 次の行動) # エージェントの移動記録等を想定
        !継続フラグ && break # ループを途中で抜ける

        行動!(環境, 次の行動) # 行動をとって次の状態に遷移

        継続フラグ = AfterAction(フック, 環境, 方策)
        !継続フラグ && break # ループを途中で抜ける
    end
...
# ╔═╡ cf2e060f-9387-457a-b564-21788ab1991c
...
# ╔═╡ c25ac9c1-22b9-4ad0-9a58-042e12ed5941
on_end!(hook::ComposeHook, env, policy) = map(hook.hooks) do h
	on_end!(h, env, policy)
end

# ╔═╡ 59c241e7-0222-4a47-b44f-0acaa60abac1
"""
...
# ╔═╡ 22fa0eae-99da-4706-b878-1480e2eab17c
env, policy, hook = (() -> begin
	env = MazeEnv()
...
# ╔═╡ 884c3efc-5db5-4abf-be4a-6b4335081f14
save_anim(env, hook.records, "img/Maze-01-random.gif")

# ╔═╡ 088a8276-6f78-423e-ab5e-f45e7cecad81
LocalResource("img/Maze-01-random.gif")

# ╔═╡ 6fde603a-83a0-47e5-9252-0e997c1ea598
...
[[deps.NaNStatistics]]
deps = ["IfElse", "LoopVectorization", "Static"]
git-tree-sha1 = "b511bf0ee14bb73e22d100164bb426c35d86291c"
