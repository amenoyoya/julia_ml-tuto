...
begin
    ...
    @testset "行動選択処理(::プレイヤー)" begin
        勇者 = RPG.キャラクター{RPG.プレイヤー}("勇者", 30, 10, 10)

        @testset "1 が入力された場合: 攻撃コマンド" begin
            # RPG.prompt() 関数を上書きすることで標準入力の自動テストが可能
            RPG.prompt(message::String) = "1"

            @test RPG.行動選択処理(RPG.toプレイヤーorモンスター(勇者)) === :攻撃
        end
        ...
        @testset "2 が入力された場合: 大振りコマンド" begin
            ...
            @test RPG.行動選択処理(RPG.toプレイヤーorモンスター(勇者)) === :大振り
        end
        ...
        @testset "上記以外が入力された場合: DomainError" begin
            ...
            @test_throws KeyError RPG.行動選択処理(RPG.toプレイヤーorモンスター(勇者))
        end
    end

    @testset "行動選択処理(::モンスター)" begin
        モンスター = RPG.キャラクター{RPG.モンスター}("モンスター", 30, 10, 10)

        @testset "攻撃コマンドが選択されること" begin
            @test RPG.行動選択処理(RPG.toプレイヤーorモンスター(モンスター)) === :攻撃
        end
    end
    ...
    @testset "ダメージ処理!" begin
        モンスター = RPG.キャラクター{RPG.モンスター}("モンスター", 100, 10, 10)
    ...
    @testset "大振り攻撃処理!" begin
        勇者 = RPG.キャラクター{RPG.プレイヤー}("勇者", 50, 30, 20)
        モンスター = RPG.キャラクター{RPG.モンスター}("モンスター", 100, 40, 10)
